...
const Dashboard: React.FC = () => {
  ...
  const filteredTasks = useMemo(() => {
    ...
    if (assigneeFilter !== 'all') {
      // FIX: The value from the select is a string, parse it to a number for comparison
      result = result.filter(task => task.assigneeId === parseInt(assigneeFilter, 10));
    }

    // Creator filter
    if (creatorFilter !== 'all') {
        // FIX: The value from the select is a string, parse it to a number for comparison
        result = result.filter(task => task.creatorId === parseInt(creatorFilter, 10));
    }
  ...
  const { toDo, inProgress, overdue, done } = useMemo(() => {
    const isCurrentlyOverdue = (task: Task) => new Date() > new Date(task.deadline) && task.status !== Status.Done;
    const wasSubmittedLate = (task: Task) =>
      task.status === Status.Done &&
      task.submittedAt &&
      new Date(task.submittedAt) > new Date(task.deadline);
    
    return {
      toDo: filteredTasks.filter(t => t.status === Status.ToDo && !isCurrentlyOverdue(t)),
      inProgress: filteredTasks.filter(t => t.status === Status.InProgress && !isCurrentlyOverdue(t)),
      overdue: filteredTasks.filter(t => isCurrentlyOverdue(t) || wasSubmittedLate(t)),
      done: filteredTasks.filter(t => t.status === Status.Done && !wasSubmittedLate(t)),
    };
  }, [filteredTasks]);
