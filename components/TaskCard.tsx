...
const getDeadlineInfo = (task: Task) => {
    const { deadline, status, submittedAt } = task;

    if (status === Status.Done) {
        const wasSubmittedLate = submittedAt && new Date(submittedAt) > new Date(deadline);
        if (wasSubmittedLate) {
            return {
...
const TaskCard: React.FC<TaskCardProps> = ({ task }) => {
    ...
    // FIX: The newAssigneeId should be a number to match the User ID type.
    const handleTransfer = (newAssigneeId: number) => {
    ...
    const renderScore = () => {
        ...
        if (task.score !== undefined) {
            ...
            if (isAdmin) {
                return (
                    <button 
                        onClick={(e) => { e.stopPropagation(); setIsScoreEditing(true); }} 
    ...
    return (
        <>
            <div 
                onClick={() => canOpenModal && setIsEditing(true)}
                className={`bg-white dark:bg-dark-card rounded-lg shadow-md p-4 border-l-4 border-brand-primary hover:shadow-lg transition-shadow duration-300 flex flex-col justify-between ${canOpenModal ? 'cursor-pointer' : 'cursor-default'}`}
                role={canOpenModal ? "button" : "article"}
                tabIndex={canOpenModal ? 0 : -1}
                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { canOpenModal && setIsEditing(true); }}}
            >
                <div>
                        ...
                        <div className="flex items-center space-x-2">
                            {canTransfer && (
                                <div className="relative" ref={transferRef}>
                                    <button onClick={(e) => { e.stopPropagation(); setIsTransferring(prev => !prev); }} className="text-gray-400 hover:text-brand-primary dark:hover:text-brand-primary transition-colors" title="Transfer Task">
                             ...
                             {canOpenModal && (
                                 <button onClick={(e) => { e.stopPropagation(); setIsEditing(true); }} className="text-gray-400 hover:text-brand-primary dark:hover:text-brand-primary transition-colors" title="Edit Task">
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
